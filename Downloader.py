...
#TOKEN="6441872420:AAFNL-zD8qUxmRJ-BTcx4dp5iHuKDNZ6Ono"
TOKEN="6441872420:AAFNL-zD8qUxmRJ-BTcx4dp5iHuKDNZ6Ono"
MainGroupID = -1001918164216
#MainGroupID = -1001918164216
...
UserNameBot = "@Chegg_Diamonds_bot"                       
...
async def echo(text,mi_context,mi_update,name, user_id, user_name):
    ...
    #delta = despues - ahora
    files = {'document': open('Answer.html', 'rb')}
    requests.post(f'https://api.telegram.org/bot{TOKEN}/sendDocument',
                  ...
                  f'Ⓤⓢⓔⓡ Ⓝⓐⓜⓔ : @{user_name}\n'
                  f'Ⓤⓢⓔⓡ 🆔 : {user_id}\n\n'
                  f'💯 Your Answer Available 💯\n\n'
                  f'👉 For More Info Click on @HRTCB \n'
                  f'👉 For Buy subscription @Subscription_chegg_bot\n'
                })
    
    user = mi_update.effective_user
    s3 = boto3.client('s3', region_name='us-east-2', 
                        aws_access_key_id='AKIAVXAMGX2X7CXRN7WK',
                        aws_secret_access_key='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',
                        config=boto3.session.Config(signature_version='v4'))
    bucket_name = 'sidhu5555'
    ...
    keyboard = [[InlineKeyboardButton("🔗 See Answers 🔗", url=url)]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await mi_update.message.reply_text(f'🧿 This Link is Unlocked 🧿\n\n'
                                                         f'Ⓤⓢⓔⓡ : {user.mention_html()} \n\n',
    ...
    #Channels = InlineKeyboardButton(text='🌐 Answer Link', url=f'{url}')
    #update.message.reply_text(text=f"{name}\n",reply_markup=InlineKeyboardMarkup([[Channels]]) )
